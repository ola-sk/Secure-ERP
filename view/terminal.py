...
def are_all_items_strings(a_list: list or tuple) -> bool:
	for item in a_list:
...
def get_table_width(min_widths, separator, padding_size):
	...
	table_structure_width = \
		len(separator) \
		+ (2 * number_of_columns * padding_size) \
		+ (number_of_columns * len(separator))
...
def get_inputs(labels: list) -> list or None:
	"""Gets a list of string inputs from the user.

	Args:
		labels: list - the list of the labels to be displayed before each prompt
	"""
	if labels or not labels:
