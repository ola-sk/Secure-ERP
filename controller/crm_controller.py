	...
	try:
		...
		for header in CUSTOMER_TABLE_HEADERS:
			if header.lower() == "id" and len(new_customers_data) == 0:
				...
				raise ValueError(
					"The new customer data at the time of writing is intended to be empty before filling out.")
				...
				while not is_correct_email_entered:
					...
					if verify_email(email):
						...
						view.print_error_message(
							"The e-mail address could not be verified. Please try entering it again.")
			elif header.lower() == "subscribed":
				is_correct_subscription_status_entered = False
				while not is_correct_subscription_status_entered:
					get_subscription_status = view.get_input(
						"Is subscribed to the newsletter? yes('yes', 'tak', 'y', '1')/ no('no', 'nie', 'n', '0')")
					...
					if is_subscription_status_valid:
						...
						if is_subscribed is not None:
							...
							raise TypeError(
								"Expected the subscription status, but got None. "
		...
		if helpers.is_number_of_fields_valid(new_customers_data, CUSTOMER_TABLE_HEADERS):
			...
			if len(new_customers_data) < len(CUSTOMER_TABLE_HEADERS):
				raise IndexError(
					"There are fields missing in customer's data. Try adding again providing the expected data.")
			elif len(new_customers_data) > len(CUSTOMER_TABLE_HEADERS):
				raise IndexError(
					"Number of fields in the customer's data is higher than expected. "
