		...
		if isinstance(table, list) and isinstance(column_index, int):
			if column_index >= 0:
				if not table_check_for_minimum_number_of_columns(table, column_index):
					raise KeyError(
						"Warning when trying to fetch column data: Inconsistent table data. Result of the "
				...
				raise ValueError(
					"Error when trying to fetch column data. Column index out of range.")
		else:
			raise TypeError(
				"Error when trying to fetch column data. Either table or index of an inappropriate type.")
